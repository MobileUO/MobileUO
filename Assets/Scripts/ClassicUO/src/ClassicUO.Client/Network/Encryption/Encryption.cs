    ...
    internal sealed class EncryptionHelper
    {
        ...
        private static (EncryptionType, uint[]) CalculateEncryption(ClientVersion version)
        {
            if (version == ClientVersion.CV_200X)
            {
                ...
                return (EncryptionType.BLOWFISH__2_0_3, new uint[3] { 0x2D13A5FC, 0x2D13A5FD, 0xA39D527F });
            }
            ...
            switch (version)
            {
                ...
                case < (ClientVersion)((1 & 0xFF) << 24 | (25 & 0xFF) << 16 | (35 & 0xFF) << 8 | 0 & 0xFF):
                    return (EncryptionType.OLD_BFISH, new uint[3] { key1, key2, key3 });
                case (ClientVersion)((1 & 0xFF) << 24 | (25 & 0xFF) << 16 | (36 & 0xFF) << 8 | 0 & 0xFF):
                    return (EncryptionType.BLOWFISH__1_25_36, new uint[3] { key1, key2, key3 });
                case <= ClientVersion.CV_200:
                    return (EncryptionType.BLOWFISH, new uint[3] { key1, key2, key3 });
                case <= (ClientVersion)((2 & 0xFF) << 24 | (0 & 0xFF) << 16 | (3 & 0xFF) << 8 | 0 & 0xFF):
                    return (EncryptionType.BLOWFISH__2_0_3, new uint[3] { key1, key2, key3 });
                default:
                    return (EncryptionType.TWOFISH_MD5, new uint[3] { key1, key2, key3 });
            }
        }
